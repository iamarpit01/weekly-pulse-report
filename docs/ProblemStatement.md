...
We are building an automated weekly “pulse” that turns public Google Play reviews for Groww into a one-page insight report and delivers it to stakeholders through Google Workspace. We will use an external, provided MCP (Model Context Protocol) server so that writes to Google Docs and Gmail go through the dedicated MCP server—not ad hoc API calls inside the agent.

**Supported product:** Groww.
...
- **Ingest** public reviews from the last 8–12 weeks (configurable window) from Google Play (scraper-based) for Groww.
...
- **Deliver outputs** only through the provided Google Workspace MCP server:
...
| :--- | :--- |
| **Data retrieval** | Ingestion module (Play Store) |
...
*The agent is an MCP client connecting to the provided MCP server; it does not embed Google credentials or call the Docs/Gmail REST APIs directly for delivery.*
